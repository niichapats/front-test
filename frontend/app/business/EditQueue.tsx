...
const EditQueue = ({queue}) => {
  ...
  const openModal = () => {
    setIsModalOpen(true);
    const modal = document.getElementById('edit_queue_modal');
    if (modal) {
        modal.showModal();
    }
  }
        ...
        <div className="modal-box">
            ...
            <form method="dialog">
                <div className='flex space-x-3'>
                    <button className='btn btn-primary' onClick={handleAddClick}>Save</button>
                    <button className='btn'>Delete</button>
                </div>
            </form>
        </div>
        </dialog>
        <button className="btn bg-transparent border-none p-0" onClick={openModal} key={queue.id}>
